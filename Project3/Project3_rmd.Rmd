...
$$ = (Y^TY - 2*Y^TX\beta+\beta^TX^TX\beta+\beta^TX^TX\beta\ \ \text{(If A is a 1x1 A =}A^T.Thus,\ \beta^TX^TY=Y^TX\beta)$$ 

$$= Y^TY +(-2)*(X^TY)^T\beta+\beta^TX^TX\beta+\beta^TX^TX\beta$$
$$\text{This tells us that}\ constant =Y^TY,\ C=-X^TY,\ and\ Q=\frac{1}{2}*X^TX$$
